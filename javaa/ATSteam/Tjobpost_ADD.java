...
import org.openqa.selenium.firefox.FirefoxDriver; // 🔄 Import this for Firefox
...
public class Tjobpost_ADD {
    ...
    public class Dashboard {
        ...
        @BeforeClass
        public void setUp() {
            // If geckodriver is not in your PATH, uncomment and set the path below:
            // System.setProperty("webdriver.gecko.driver", "path/to/geckodriver.exe");

            driver = new FirefoxDriver(); // ✅ Use FirefoxDriver
            driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));
            driver.manage().window().maximize();
            wait = new WebDriverWait(driver, Duration.ofSeconds(15)); // Save the wait instance if needed
        }
	    @Test
		public void testInvalidUsernameOrPassword() throws InterruptedException {
	        ...
	        WebElement emailField = driver.findElement(By.xpath("/html/body/div/div[2]/div/div[1]/div/div/form/input"));
	        System.out.println(" email  filled successfully...");
	        Thread.sleep(3000);
	        emailField.sendKeys("purvi@insightfultechno.com");
	        ...
	        passwordField.sendKeys("Purvi@123");
	        ...
	        System.out.println("add to jobpost button clicked");
		       ...
		       System.out.println("jobcode filled successfully");
		        ...
		        jobTitlElement.sendKeys("firetwo");
...
//		        WebElement cityElement = driver.findElement(By.xpath("/html/body/div[1]/div/div/div[5]/form/div[5]/div[1]/div/div/div/select/option[3]"));
//		        cityElement.click();
//		        System.out.println("city selected");
		        
		     // ✅ Step 1: Find the <select> element only (not the <option>)
		        WebElement dropdown = driver.findElement(By.xpath("/html/body/div[1]/div/div/div[5]/form/div[5]/div[1]/div/div/div/select"));

		        // ✅ Step 2: Scroll the <select> into view
		        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", dropdown);

		        // ✅ Step 3: Use Select class on the <select>
		        Select select = new Select(dropdown);
		        select.selectByIndex(3); // Selects 4th option
		    ...
		    // ✅ Step 1: Find the <select> element — NOT <option>
		       WebElement countryDropdown = driver.findElement(By.xpath("/html/body/div[1]/div/div/div[5]/form/div[4]/div[2]/div/div/select"));

		       // ✅ Step 2: Scroll the <select> into view
		       ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", countryDropdown);

		       // ✅ Step 3: Use Select class to select the 2nd option (index 1)
		       Select country = new Select(countryDropdown);
		       country.selectByIndex(1); // Selects option[2] (index starts at 0)
...
//		       System.out.println("country selected");
		  ...
		  /// ✅ Step 1: Find the <select> element (NOT the <option>)
		     WebElement clientbill = driver.findElement(By.xpath("/html/body/div[1]/div/div/div[5]/form/div[2]/div[3]/div/div[1]/select"));

		  // ✅ Step 2: Scroll the <select> into view
		  ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", clientbill);

		  // ✅ Step 3: Create a Select object and choose the desired option
		  Select selecct = new Select(clientbill);
		  selecct.selectByIndex(2); // Selects option[3], because index starts at 0
...
//		    System.out.println("pay rate salary clicked");
		 ...
		 // ✅ Step 1: Find the <select> element (NOT the <option>)
		 // ✅ Step 1: Find the <select> element — NOT <option>
		    WebElement industryDropdown = driver.findElement(By.xpath("//*[@id='kt_app_content']/form/div[14]/div[1]/div/div/select"));

		    // ✅ Step 2: Scroll into view
		    ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center'});", industryDropdown);

		    // ✅ Step 3: Create Select object and select the 3rd option (index 2)
		    Select industrySelect = new Select(industryDropdown);
		    industrySelect.selectByIndex(2); // Selects <option>[3]
